...
import { LoadingOutlined } from "@components/antd-client/icons";
import { Spin } from "@components/antd-client";
...
const CreateAccount = (context) => {
  ...
  const [showTokenLoading, setShowTokenLoading] = useState<boolean>(false);
  ...
  const fetchInvitation = async () => {
    ...
    try {
      const response = await api.get(`/cowork/employees/invites/${token}`);
      if (response?.data?.result?.userExists) {
        await api.post(`/cowork/employees/invites/${token}`, "");
        toast.success("Convite aceito!");
        router.push("/login");
        return;
      }
    } catch (error) {
      console.log("error fetchInvitation", error);
  ...
  const handleSubmit: SubmitHandler<FormData> = async (data) => {
    try {
      ...
      await api.post(`/cowork/employees/invites/${token}`, body);
  ...
  return (
    <Container>
      ...
      {showTokenLoading ? <Spin indicator={antIcon} /> : GenerateForm()}
