...
function ClientModal({ client, onClose }: { client?: any; onClose: () => void }) {
  ...
  return (<div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center" onClick={onClose}>
    <div className="bg-surface rounded-xl max-sm:rounded-none shadow-2xl max-w-3xl w-full max-sm:w-full mx-4 max-sm:mx-0 max-h-[90vh] max-sm:max-h-full max-sm:h-full overflow-y-auto" onClick={e=>e.stopPropagation()}>
      ...
      <div className="p-6">
        ...
        {/* B3-R1 responsive */}<div className="overflow-x-auto"><table className="w-full min-w-[700px] text-[13px] border-collapse"><thead><tr className="text-left border-b-2 border-border text-[11px] text-outline uppercase"><th className="p-2">Service</th><th className="p-2">Room</th><th className="p-2">Period</th><th className="p-2">Rate</th><th className="p-2">Channel</th><th className="p-2">Status</th></tr></thead>
        <tbody>{contracts.map((sc:any)=>(<tr key={sc.id} className="border-b border-border"><td className="p-2 font-medium">{sc.serviceType?.name||'—'}</td><td className="p-2">{sc.roomsUnit?.display_name||'—'}</td><td className="p-2 text-[12px]">{sc.startedAt?new Date(sc.startedAt).toLocaleDateString():'—'} — {sc.endedAt?new Date(sc.endedAt).toLocaleDateString():'open'}</td><td className="p-2">${((sc.price_cents||0)/100).toFixed(2)}</td><td className="p-2"><span className={`px-2 py-0.5 rounded-full text-[11px] font-medium ${sc.billing_channel==='DIRECT'?'bg-blue-100 text-blue-800':'bg-amber-100 text-amber-800'}`}>{sc.billing_channel==='DIRECT'?'EWS (Direct)':sc.reseller?.name||'Reseller'}</span></td><td className="p-2"><span className={`px-2 py-0.5 rounded-full text-[11px] font-medium ${sc.status==='ACTIVE'?'bg-green-100 text-green-800':'bg-amber-100 text-amber-800'}`}>{sc.status}</span></td></tr>))}</tbody></table></div>
...
export default function AllClientsPage() {
  ...
  return (<Shell><Head><title>All Clients | Workeaser</title></Head>
    ...
    <div className="bg-surface border border-border rounded-lg overflow-hidden">{/* B3-R1 responsive */}<div className="overflow-x-auto"><table className="w-full min-w-[700px] text-[13px] border-collapse"><thead><tr className="bg-[#2B3450] text-white text-left text-[11px] uppercase"><th className="p-3">Client</th><th className="p-3">Company</th><th className="p-3">Email</th><th className="p-3">Phone</th><th className="p-3">Services</th><th className="p-3">Room</th><th className="p-3">Status</th><th className="p-3 w-10"></th></tr></thead>
    <tbody>{clients.map((c:any)=>{const sc=c.serviceContracts||[];const f=sc[0];return(<tr key={c.id} className="border-b border-border cursor-pointer hover:bg-surface-container-low"><td className="p-3 font-medium" onClick={()=>setSelected(c)}><div className="flex items-center gap-2"><div className="w-8 h-8 rounded-full bg-primary/20 flex items-center justify-center text-[11px] font-semibold text-primary">{(c.contact_first_name||'?')[0]}{(c.contact_last_name||'?')[0]}</div>{c.contact_first_name} {c.contact_last_name}</div></td><td className="p-3">{c.company_name||'—'}</td><td className="p-3">{c.company_email||c.contact_email||'—'}</td><td className="p-3">{c.company_phone||c.contact_phone||'—'}</td><td className="p-3">{sc.map((s:any)=>s.serviceType?.name).filter(Boolean).join(', ')||'—'}</td><td className="p-3">{f?.roomsUnit?.display_name||'—'}</td><td className="p-3"><span className={`px-2 py-0.5 rounded-full text-[11px] font-medium ${f?.status==='ACTIVE'?'bg-green-100 text-green-800':'bg-amber-100 text-amber-800'}`}>{f?.status||'No services'}</span></td><td className="p-3"><span className="material-symbols-outlined text-[18px] text-outline cursor-pointer" onClick={(e)=>{e.stopPropagation();setSelected(c)}}>more_vert</span></td></tr>)})}</tbody></table></div>
...
function AddClientModal({ onClose }: { onClose: () => void }) {
  ...
  return (<div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center" onClick={onClose}><div className="bg-surface rounded-xl max-sm:rounded-none shadow-2xl max-w-3xl w-full max-sm:w-full mx-4 max-sm:mx-0 max-h-[90vh] max-sm:max-h-full max-sm:h-full overflow-y-auto" onClick={e=>e.stopPropagation()}><div className="p-6 border-b border-border flex justify-between items-center"><h2 className="text-[18px] font-semibold text-[#2B3450]">Add Client</h2><button onClick={onClose} className="text-outline hover:text-on-surface text-[24px] leading-none">&times;</button></div><div className="p-6"><form onSubmit={submit}>
