...
/**
 * Parse CORS_ALLOWED_ORIGINS env var (comma-separated list).
 * Falls back to APP_URL if not set. Empty/missing disables cross-origin completely
 * via `origin: false`.
 */
const allowedOriginsRaw = (Env.get('CORS_ALLOWED_ORIGINS', '') as string).trim();
const fallback = (Env.get('APP_URL', '') as string).trim();
const allowedOrigins = (allowedOriginsRaw || fallback)
  .split(',')
  .map((o) => o.trim())
  .filter(Boolean);
...
const corsConfig: CorsConfig = {
  ...
  */
  ...
  |
  */
  // SECURITY: explicit allowlist. To enable cross-origin requests in dev/staging,
  // set CORS_ALLOWED_ORIGINS in .env to a comma-separated list (e.g. "https://app.workeaser.com,https://stage.app.workeaser.com").
  // If neither CORS_ALLOWED_ORIGINS nor APP_URL is set, cross-origin requests are denied.
  origin: function (requestOrigin: string) {
