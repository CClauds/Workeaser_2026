...
import ChatService from 'App/Services/Cowork/ChatService';
import AppError from 'App/Utils/AppError';
import { responseWithError, responseWithSuccess } from 'App/Utils/ResponseApi';
import ChatValidatorCowork from 'App/Validators/Cowork/Chat/ChatValidatorCowork';
...
export default class ChatController {
  ...
  async firstOrCreateChat({ request, response, auth }: HttpContextContract) {
    response.header('Cache-Control', 'no-cache, no-store');
    try {
      const payload = await request.validate(ChatValidatorCowork);
      const user = auth.user;
  ...
  async showChatMessages({ params, response, auth }: HttpContextContract) {
    ...
    try {
      ...
      const messageData = await ChatService.showChatMessages(user, id);

      return responseWithSuccess(response, messageData);
    } catch (error) {
